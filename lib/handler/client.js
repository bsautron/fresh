...
export default {
  get: (req, res) => {
    models.ClientShemas.find((err, clients) => {
      if (err) return res.json(response.fail(0))

      let rr = {};
      for (let c of clients) if (c && c.name) rr[c.name] = models.ClientShemas.getInfos(c);
      res.json(response.success(rr));
    });
  },
  post: (req, res) => {
    let client = new models.ClientShemas({
      name: req.body.name,
      key: req.body.key,
    ...
    .catch((err) => {
      res.json(response.success({message: 'response fail in comming!'}));
